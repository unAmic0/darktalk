...
function getTemplate(title, msg, value, buttons) {
  const encodedMsg = DOMPurify.sanitize(msg).replace(/\n/g, "<br>");

  return `<main class="page">
        <header>${DOMPurify.sanitize(title)}</header>
        <div class="content-area">${encodedMsg}${DOMPurify.sanitize(value)}</div>
        <div class="action-area">
            <div class="button-strip">
                ${DOMPurify.sanitize(parseButtons(buttons))}
            </div>
        </div>
    </main>`;
}
...
function parseButtons(buttons) {
  ...
  const parse = currify(
    (buttons, name, i) => `<button
            tabindex=${DOMPurify.sanitize(i)}
            data-name="js-${DOMPurify.sanitize(name).toLowerCase()}">
            ${DOMPurify.sanitize(buttons[name])}
        </button>`,
  );
...
const query = currify((element, name) =>
  element.querySelector(`[data-name="js-${DOMPurify.sanitize(name)}"]`),
);
